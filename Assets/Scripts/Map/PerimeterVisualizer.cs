...
public class PerimeterVisualizer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Tab) || TileEditor.instance.currentTilemapLayer == 4)
        {
        ...
        else if (!Input.GetKey(KeyCode.Tab) && TileEditor.instance.currentTilemapLayer != 4)
        {
            foreach (GameObject child in perimeters)
            {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (go == perimeters[0])
                go.transform.position = new Vector3(go.transform.position.x,
                    Mathf.Clamp(Mathf.RoundToInt(currentMousePos.y - mousePos.y), 501 - 108, 501), 0);
            else if (go == perimeters[1])
                go.transform.position = new Vector3(go.transform.position.x,
                    Mathf.Clamp(Mathf.RoundToInt(currentMousePos.y - mousePos.y), -607, -607 + 108));
            else if (go == perimeters[2])
                go.transform.position =
                    new Vector3(Mathf.Clamp(Mathf.RoundToInt(currentMousePos.x - mousePos.x), 608 - 108, 608),
                        go.transform.position.y, 0);
            else if (go == perimeters[3])
                go.transform.position =
                    new Vector3(Mathf.Clamp(Mathf.RoundToInt(currentMousePos.x - mousePos.x), -500, -500 + 108),
                        go.transform.position.y, 0);
